...
## 联系人经历系统
- 工具调用兼容层在 `deepseek.ts` 统一归一化：支持原生 `tool_calls`、旧式 `function_call`、被兼容网关序列化进 `message.content` 的 `tool_calls`，并补齐缺失 id、将对象形式 arguments 转为 JSON 字符串、丢弃未在请求白名单内的工具。流式工具参数同时兼容增量分片和累计分片，不要在业务层重复造一套解析。
- 私聊正文与行动决策的强制工具调用都会对无效参数最多重试一次；人物生成、女娲表单和群聊工具也各自有有界重试与本地校验。不要把无效的 `decided=true` 静默降级成无行动。
- 联系人创建协议会根据人设同时生成 `wardrobe` 和 `currentOutfit`，写入联系人的初始衣橱/穿着快照。聊天引擎会把当前穿着和衣橱注入私聊、群聊提示词；只有角色实际完成穿上、脱下或换装后才能调用 `change_outfit`/`outfit_change`，必须提交变更后的完整穿着；讨论、建议、未来打算、拒绝或仅描述原有穿着不能改状态。新出现的衣物会自动并入衣橱，世界分支会隔离并恢复对应衣物快照。
