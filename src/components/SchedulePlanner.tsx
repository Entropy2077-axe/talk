...
import { buildScheduleOptimizationPrompt, parseOptimizedSchedule } from '../lib/scheduleOptimization'
...
export function SchedulePlanner({ contact, settings, memories }: { contact: Contact; settings: AppSettings; memories: ContactMemory[] }) {
  ...
  async function optimize() {
    ...
    if (!instruction) { setError('请先输入要如何修改日程'); return }
    ...
    try {
      ...
      const raw = await chatCompletionText({ apiKey: settings.apiKey, baseUrl: settings.baseUrl, provider: settings.aiProvider, model: settings.utilityModel || settings.model, purpose: 'persona', jsonMode: true, temperature: 0.2, maxTokens: 3000, messages: [{ role: 'system', content: prompt }, { role: 'user', content: `请按照下面的玩家指令修改固定周日程，并返回修改后的完整固定周日程：\n${instruction}` }] })
      const optimized = parseOptimizedSchedule(raw, leafLocations)
      setCandidate(optimized)
      setShowAiEditor(false)
    } catch (cause) { setError(cause instanceof Error ? cause.message : '日程修改失败') } finally { setOptimizing(false) }
  }
  ...
  return <section className="mx-3 mt-4 rounded-[var(--ui-radius-card)] bg-[var(--ui-surface)] px-4 py-4 shadow-[var(--ui-shadow)]">
    <div className="mb-2 flex items-center justify-between gap-2"><div className="flex items-center gap-2"><h3 className="text-xs font-medium text-gray-400">日程表</h3><button type="button" onClick={() => { setAiInstruction(''); setError(''); setShowAiEditor(true) }} disabled={optimizing} className="rounded-lg bg-gray-100 px-2 py-1 text-[11px] text-gray-700 disabled:opacity-50">AI 修改</button></div><div className="flex items-center gap-1 text-xs text-gray-600"><button type="button" aria-label="上一周" onClick={() => setWeekStart((value) => new Date(value.getFullYear(), value.getMonth(), value.getDate() - 7))} className="flex h-7 w-7 items-center justify-center rounded-md border border-gray-200"><ChevronLeft size={16} /></button><span className="min-w-20 text-center font-medium">{title}</span><button type="button" aria-label="下一周" onClick={() => setWeekStart((value) => new Date(value.getFullYear(), value.getMonth(), value.getDate() + 7))} className="flex h-7 w-7 items-center justify-center rounded-md border border-gray-200"><ChevronRight size={16} /></button></div></div>
    ...
    {showAiEditor && <AiScheduleEditor instruction={aiInstruction} busy={optimizing} error={error} onInstructionChange={(value) => { setAiInstruction(value); if (error) setError('') }} onCancel={() => { if (!optimizing) { setShowAiEditor(false); setError('') } }} onSubmit={() => void optimize()} />}
...
function AiScheduleEditor({ instruction, busy, error, onInstructionChange, onCancel, onSubmit }: { instruction: string; busy: boolean; error: string; onInstructionChange: (value: string) => void; onCancel: () => void; onSubmit: () => void }) {
  return <div className="absolute inset-0 z-40 flex items-end bg-black/40" onClick={onCancel}><div className="w-full rounded-t-2xl bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]" onClick={(event) => event.stopPropagation()}><h3 className="text-base font-medium text-gray-900">让 AI 修改日程</h3><p className="mt-1 text-xs leading-5 text-gray-400">用自然语言说明要增加、删除或调整什么。AI 会先生成完整的新日程，确认后才会替换；特殊安排不会改动。</p><textarea autoFocus value={instruction} maxLength={1000} onChange={(event) => onInstructionChange(event.target.value)} rows={5} disabled={busy} placeholder="例如：周一到周五改成早上九点上班、下午六点下班；每周三晚上增加两小时健身，周末不要安排工作。" className="mt-3 w-full resize-none rounded-xl border border-gray-200 px-3 py-2.5 text-sm leading-relaxed outline-none focus:border-gray-400 disabled:bg-gray-50" />{error && <p className="mt-2 text-xs text-red-500">{error}</p>}<div className="mt-3 flex gap-2"><button type="button" onClick={onCancel} disabled={busy} className="flex-1 rounded-xl bg-gray-100 py-2.5 text-sm text-gray-700 disabled:opacity-50">取消</button><button type="button" onClick={onSubmit} disabled={busy || !instruction.trim()} className="flex-1 rounded-xl bg-gray-900 py-2.5 text-sm text-white disabled:opacity-50">{busy ? '修改中…' : '生成修改方案'}</button></div></div></div>
}
