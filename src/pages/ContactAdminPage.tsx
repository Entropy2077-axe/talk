...
export function ContactAdminPage() {
  ...
  if (!settings.adminModeEnabled) return <div className="flex h-[var(--app-height)] flex-col overflow-hidden bg-[var(--ui-bg)]"><TopBar title="管理员编辑" showBack /><p className="p-8 text-center text-sm text-[var(--ui-text-3)]">请先开启管理员模式</p></div>
  if (!contact || !draft || !promptDraft) return null

  return <div className="flex h-[var(--app-height)] flex-col overflow-hidden bg-[var(--ui-bg)]">
    ...
    <div className="flex-1 space-y-3 overflow-y-auto px-3 pb-28 pt-3 [&>section]:!mt-0 [&>section]:rounded-[var(--ui-radius-card)] [&>section]:shadow-[var(--ui-shadow)]">
      <section className="bg-[var(--ui-surface)] px-4 py-4">
        <p className="text-xs font-medium text-[var(--ui-text-3)]">当前编辑对象</p>
        <h1 className="ui-font-display mt-1 text-lg font-semibold text-[var(--ui-text)]">{draft.remark || draft.name}</h1>
        <p className="mt-2 text-xs leading-relaxed text-[var(--ui-text-3)]">先处理人物和关系资料；提示词、协议与真实后台数据属于低频高级内容，修改后统一在底部保存。</p>
        <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px]"><span className="rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-2)] px-2 py-2 text-[var(--ui-text-2)]">身份与人设</span><span className="rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-2)] px-2 py-2 text-[var(--ui-text-2)]">关系与生活</span><span className="rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-2)] px-2 py-2 text-[var(--ui-text-2)]">高级数据</span></div>
      </section>
    ...
    <div className="absolute inset-x-0 bottom-0 border-t border-[var(--ui-border)] bg-[var(--ui-surface)] px-4 py-3 pb-[calc(.75rem+env(safe-area-inset-bottom))]"><button type="button" onClick={() => void saveAll()} className="w-full rounded-[var(--ui-radius-control)] bg-[var(--ui-action)] py-3 text-sm font-medium text-[var(--ui-on-action)]">保存全部修改</button>{status && <p className={`mt-2 text-center text-xs ${status.startsWith('已') || status.includes('载入') ? 'text-[var(--ui-success-ink)]' : 'text-[var(--ui-danger-ink)]'}`}>{status}</p>}</div>
  </div>
}
