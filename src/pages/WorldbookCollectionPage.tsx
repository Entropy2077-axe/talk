...
export function WorldbookCollectionPage() {
  ...
  if (!collection) return <div className="flex h-[var(--app-height)] flex-col bg-[var(--ui-bg)]"><TopBar title="世界观" showBack onBack={goBack}/><div className="flex flex-1 items-center justify-center text-sm text-[var(--ui-text-3)]">加载中或世界不存在</div></div>
  return <div className="relative flex h-[var(--app-height)] flex-col overflow-hidden bg-[var(--ui-bg)]">
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-8">
      <section className="mt-3 rounded-[var(--ui-radius-card)] bg-[var(--ui-surface)] p-4 shadow-[var(--ui-shadow)]"><div className="flex justify-between gap-3"><div><p className="text-xs font-medium text-[var(--ui-text-3)]">当前世界正史</p><h1 className="ui-font-display mt-1 text-lg font-semibold text-[var(--ui-text)]">{collection.name}</h1><p className="mt-1 text-xs text-[var(--ui-text-3)]">最新剧情含 {storyContacts} 个联系人状态 · {storyGroups} 个群聊</p></div><button type="button" onClick={async () => { const name = window.prompt('世界观名称', collection.name)?.trim(); if (name) await db.worldbookCollections.update(collection.id, { name, updatedAt: Date.now() }) }} className="self-start text-xs text-[var(--ui-link)]">重命名</button></div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-[10px]"><div className="rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-2)] p-2 text-[var(--ui-text-2)]">正史条目<br/><b>{entries.length}</b></div><div className="rounded-[var(--ui-radius-control)] bg-[var(--ui-warning-soft)] p-2 text-[var(--ui-warning-ink)]">每轮常驻<br/><b>{formatEstimatedTokens(estimateWorldbookTokens(permanent))}</b></div><div className="rounded-[var(--ui-radius-control)] bg-[var(--ui-info-soft)] p-2 text-[var(--ui-info-ink)]">按需触发<br/><b>{formatEstimatedTokens(estimateWorldbookTokens(triggered))}</b></div></div>{estimateWorldbookTokens(permanent) > 4000 && <p className="mt-2 rounded-[var(--ui-radius-control)] bg-[var(--ui-warning-soft)] px-3 py-2 text-xs text-[var(--ui-warning-ink)]">常驻内容超过4000 Token，可能明显挤占聊天记录和人物记忆。</p>}<div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => open()} className="rounded-[var(--ui-radius-control)] bg-[var(--ui-action)] py-2.5 text-sm text-[var(--ui-on-action)]">手写条目</button><button type="button" onClick={() => setLibraryOpen(true)} className="rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-2)] py-2.5 text-sm text-[var(--ui-text-2)]">从资料库加入</button></div></section>
      {message && <p className="mt-3 rounded-lg bg-white px-3 py-2 text-xs leading-relaxed text-gray-500">{message}</p>}
      <div className="sticky top-0 z-10 -mx-4 mt-4 bg-[var(--ui-bg)] px-4 py-2"><h2 className="mb-2 text-sm font-semibold text-[var(--ui-text)]">正史条目</h2><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索正史标题、关键词或正文" className="w-full rounded-[var(--ui-radius-control)] border border-[var(--ui-border)] bg-[var(--ui-surface)] px-3 py-2 text-sm"/></div>
