 ...
 * never editable — this is what makes the JSON bubble/sticker/link protocol
 * reliable, so it must not drift when a user edits their style prompt.
 */
...
export function buildSystemPrompt(opts: {
  ...
  const protocol = FIXED_PROTOCOL_PROMPT.replace('{{STICKERS}}', stickersText).replace(
    '{{LINKS}}',
    linksText,
  )
  ...
  return [opts.stylePrompt, protocol, personaSection, memorySection].join('\n\n')
}
...
export function parsePersonaGeneration(raw: string): { name: string; persona: string } | null {
  let text = raw.trim()
