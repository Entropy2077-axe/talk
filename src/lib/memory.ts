...
function buildMemoryUpdatePrompt(existingFacts: string, existingStyle: string): string {
  return `你是一个对话记忆整理器 任务是帮一个角色扮演AI维护它对聊天对象的记忆 你不会出现在对话里 只输出JSON 不要有其他任何文字
...
接下来会给你一批新的聊天记录（"对方"是用户 "你"是角色扮演AI自己） 请你更新上面两部分记忆 输出格式:
{"facts": "...", "style": "..."}
...
- style是这个AI应该如何调整语气和熟悉程度来更贴合这位对话对象 比如"关系变熟了 可以更随便"、"对方喜欢简短回复 你也保持简短"、"对方情绪低落时会安静倾听不追问"、"对方开玩笑比较多 可以多接梗" 这类相处细节 绝对不能改变角色的核心性格 只是语气和熟悉度上的贴合 总长度控制在150字以内
...
function parseMemoryResponse(raw: string): { facts: string; style: string } | null {
  let text = raw.trim()
  ...
  try {
    ...
    if (typeof parsed?.facts === 'string' && typeof parsed?.style === 'string') {
      return { facts: parsed.facts.trim(), style: parsed.style.trim() }
    }
  } catch {
 ...
 * memory. Silently does nothing on failure — memory is an enhancement, it
 * must never block or break the chat itself.
 */
export async function maybeUpdateMemory(
  ...
  try {
    ...
    const raw = await chatCompletion({
      ...
      model: api.model,
      messages: [
        { role: 'system', content: buildMemoryUpdatePrompt(contact.memoryFacts, contact.memoryStyle) },
