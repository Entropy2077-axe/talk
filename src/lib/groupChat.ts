...
export function buildGroupSystemPrompt(opts: {
  ...
  const speakerBlocks = opts.speakers
    .map((c, i) => {
      ...
      return `发言人${i + 1}: ${c.name}（${base}${dynamic}）\n人设: ${c.systemPrompt || '自由发挥'}${plansLine}${scheduleLine}\n了解: ${c.memoryFacts || factsFallback}\n相处: ${c.memoryStyle || styleFallback}`
    })
    .join('\n\n')
...
对方(用户): ${opts.userProfileText}${knowledgeLine}
