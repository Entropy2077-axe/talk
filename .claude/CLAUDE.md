...
### 世界书 v3：从单一 `worldview` 字符串升级为结构化关键词匹配条目系统（`lib/worldbook.ts` + `WorldSettingsPage`，新增）
旧的 `settings.worldview` 是一个自由文本字符串，每次聊天/朋友圈/群聊都原样注入整个文本——不管跟当前话题有没有关系，既浪费token又稀释了真正相关的信息密度。**世界书把它拆成多条独立条目**，每条有自己的标题、内容、触发关键词、优先级，只在跟当前上下文匹配时才被检索注入。

**数据模型**：`WorldbookEntry { id, title, content, keywords[], enabled, alwaysInclude, priority(0-100), createdAt, updatedAt }`，存在 `db.worldbookEntries` 表。`SettingsPage` 和 `WorldSettingsPage` 都有管理入口。

**检索算法是纯本地的关键词匹配，不是向量/语义搜索**（`rankWorldbookEntries()`）：
- `alwaysInclude: true` 的条目永远命中（score = 10000 + priority），用于核心世界观设定。
- 其余条目按关键词命中（精确包含匹配，`+=80`/关键词）、标题命中（`+=12`/term）、内容命中（`+=2`/term）累计加分，最终分数必须超过条目自身 priority 才算命中。
- **分词策略是针对中英文混合场景特意设计的**（`terms()`）：拉丁文按 `[a-z0-9_-]{2,}` 提取；中日韩文字先整词匹配（`{2,}` 长度的连续汉字块），再切成相邻二字 bigram 作为补充匹配单位。**这不是"分词不准确"的bug——这是一个纯前端应用，没有后端也没有嵌入模型，bigram 是中文关键词匹配在无词典条件下的经典折中方案**，比单字匹配噪音小、比整词匹配召回率高。
- 排序：score 降序 → priority 降序 → updatedAt 降序。命中条目有总字符数上限（默认5000字）防止 token 爆炸。
- 管理员模式下 `console.log` 输出命中条目列表（`[worldbook] 命中: ...`），方便调试检索效果。

**两个导出函数的职责分离是有意为之**：
- `retrieveWorldbookContext(query)` — 返回纯文本字符串，给不需要追踪元数据的调用方用（群聊 `groupChatEngine.ts`、朋友圈 `moments.ts`、生活模拟 `lifeSimulation.ts`、联系人创建 `ContactAddPage.tsx`）。
- `retrieveWorldbookTrace(query)` — 返回 `{ text, matches }` 对象，给需要记录匹配详情的调用方用（1:1 聊天 `chatEngine.ts`，匹配详情写入 `AIMessage.worldbookMatches` 字段供后续分析）。**这不是代码重复，两个函数的语义和返回值不同。**

**注入位置**：世界书文本注入到 1:1 聊天、群聊、朋友圈生成、生活模拟、联系人创建（人设生成时作为"必须遵守的世界书"）五处。统一放在 prompt 拼接顺序里"世界观/世界书"位置（说话风格之后、人物设定之前），跟旧 `worldview` 的注入位置一致。

**旧数据迁移**（`ensureLegacyWorldviewMigrated()`）：`App.tsx` 首次挂载时调用，把旧 `settings.worldview`（如果非空）写进一条 `alwaysInclude: true, priority: 100` 的 worldbook 条目（标题"旧世界设定"），然后**清空 `settings.worldview` 并设 `worldbookMigrationCompleted: true` 防止重复执行**。**`worldview` 被清空是迁移的预期行为，不是数据丢失 bug**——世界书是旧世界观的升级替代，迁移后所有读取世界书的地方（通过 `retrieveWorldbookContext`）都能检索到这条迁移条目，等价于旧行为。`settings.worldview` 字段本身保留未删除只是为了向后兼容（`AppSettings` 的 zod schema 仍然包含它），但新代码不再直接读取它。

**与旧 `worldview` 的关键差异（代码审查时容易误判的点）**：
- 旧 `worldview` 是一个字符串字段存在 `AppSettings`（localStorage），世界书是多条独立记录存在 IndexedDB。两者是不同的存储位置和数据结构，迁移是一次性的单向操作。
- `SavedWorldview`（收藏夹，`db.savedWorldviews`）和 `WorldbookEntry`（世界书条目，`db.worldbookEntries`）是两张不同的表，不要混淆。收藏夹存的是"用户喜欢的世界观草稿/快照"（手动保存+手动应用），世界书存的是"当前生效的检索条目"。
- 世界书条目可以在 `WorldSettingsPage` 直接增删改，不需要走"草稿→确认"流程——跟 AI 帮写世界观草稿那套流程是同一个页面里的两个独立功能。
...
## 目录结构速查
- `src/pages/`：MessagesPage(消息列表，含1:1+群聊+未读红点，右上角"+"发起) / ContactsPage / ContactAddPage(问卷+直接创建+AI关系设定+日程生成) / ContactCardPage(含日程展示) / ChatPage(1:1与群聊共用，含委托卡片交互) / GroupAddPage(建群) / GroupInfoPage(成员列表+解散群聊) / TodoPage / DiscoverPage(朋友圈/商城/仓库/关系网/世界设定/天眼(仅管理员模式)入口) / RelationshipsPage(用户-AI关系总览+AI-AI关系展示) / MomentsPage(朋友圈) / WorldSettingsPage(世界观自写/AI帮写/收藏夹+知识库列表/删除/手动搜索+世界书条目管理) / SkyEyePage(console日志+数据统计+设置dump) / ShopPage / WarehousePage / MePage(含货币显示) / ProfileEditPage / SettingsPage(含Tavily key+管理员模式开关) / StickersPage。
