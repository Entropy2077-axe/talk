...
- **这几个阈值都是代码常量，没有设置页UI**（冷却时长/每日上限/概率/沉默阈值），跟`REACT_PROBABILITY`/`COMMENT_SHARE`是同一类"以后觉得不对再改代码"的参数，不值得现在就做成可配置项。
