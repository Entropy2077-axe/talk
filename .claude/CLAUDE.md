...
**"第二次及以后回复都不对"bug修复记录**：根因是每一轮AI回复在数据库里是拆成好几条独立的 `role:'assistant'` 消息（分句气泡）存的，`ChatPage.runAiTurn` 之前是把 `recentHistory` 里的每一条数据库消息都1:1映射成一条 `ChatMessage`，导致第一轮过后历史记录里会出现连续好几条 `assistant` 角色消息、中间没有穿插`user`消息——破坏了绝大多数对话模型期望的user/assistant严格交替结构，导致从第二轮开始模型的理解和输出质量明显下降（这也是为什么第一轮永远正常：第一轮发请求时历史里还没有任何assistant消息）。**已修复**：`src/lib/deepseek.ts` 新增 `coalesceConsecutiveRoles()`，在真正发给API之前把连续同角色的消息合并成一条（用换行拼接），`ChatPage.runAiTurn` 组装 `chatMessages` 时统一过一遍这个函数。以后如果哪里还要把数据库里的多气泡历史转成发给模型的messages数组，记得也要过一遍这个函数，否则同样的问题会再犯。
