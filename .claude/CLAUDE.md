...
用户反馈过"退出聊天界面之后聊天无法进行了"——根因是发消息/调API/逐条揭示气泡这套逻辑原本整个长在 `ChatPage` 组件的 `useState`/`useRef` 里，组件卸载时的cleanup effect会 `abort()` 掉正在进行的请求、清空所有待触发的气泡定时器，等于用户一退出聊天页，AI的回复就被腰斩了。

**已经整体挪到 `src/lib/chatEngine.ts`，独立于任何组件的生命周期**：
- `sendMessage(conversationId, contact, settings, stickers, text)`——发消息+触发AI回复的唯一入口，`ChatPage`的输入框和委托卡片的接取/拒绝按钮都调用这一个函数。
...
- 不需要响应式的簿记（当前streamId、待触发的气泡定时器、AbortController）用模块级 `Map<conversationId, ...>` 存，同样不挂在任何组件上。
- **`ChatPage`卸载时现在什么都不清理**——没有 `abort()`、没有清定时器。只有"同一个会话又发了新消息"才会打断上一轮（`clearPending()`，插话打断机制还是那一套，没变），退出页面这个动作本身不再触发任何打断。这样退出聊天页之后，AI该怎么回复还是怎么回复，气泡照常一条条落库，回来看历史记录都在。

**如果以后要改聊天发送/回复逻辑，去改 `chatEngine.ts`，不要加回 `ChatPage.tsx` 里**——`ChatPage` 现在应该只剩UI和读取展示，一旦看到它里面又出现 `streamRef`/`timersRef`/`abortRef`/本地`aiTyping` state，说明有人把后台化以前的写法抄回来了，这是不对的。

**后台化上线后立刻炸了一次："聊天界面直接卡没了"（白屏/崩溃）**——根因不是逻辑错误，是个经典的 React18+Zustand 陷阱：`ChatPage` 订阅 `useChatEngineStore` 时写的是 `(s) => s.states[conversationId ?? ''] ?? { aiTyping: false, error: '' }`，**这个兜底对象字面量每次selector执行都会new一个新的**。只要这个会话还没有任何engine state（比如刚打开一个还没发过消息的聊天），每次渲染拿到的都是引用不同的新对象，触发了React `useSyncExternalStore` 的"getSnapshot返回值不稳定"检测，直接导致组件崩溃、整个聊天页白屏消失。**已修复**：`chatEngine.ts` 导出一个模块级单例 `DEFAULT_RUNTIME_STATE`，selector兜底时用这个稳定引用而不是现场new一个。**以后凡是写 `useXxxStore(s => s.xxx ?? 默认值)` 这种selector，默认值必须是模块级的稳定引用（提前定义好的const对象/数组），绝对不能是内联字面量**——内联字面量在选择器里天然不稳定，是这类"页面直接崩掉/白屏"故障的常见根源，写代码时要留意，review的时候也要专门检查这个模式。

## 全局通知横幅（`NotificationBanner` + `useChatUiStore`）
用户要求"发了消息会给模拟弹窗"。`useChatUiStore`（`src/store/useChatUiStore.ts`，同样不persist）存两样东西：
- `activeConversationId`——当前打开的是哪个会话，`ChatPage` mount时登记、unmount时清空。
- `notification`——当前要展示的通知内容（联系人名字/头像/预览文字/conversationId）。

`chatEngine.ts` 的 `revealBubbles()` 每次有新气泡落库时，检查 `activeConversationId !== conversationId`（也就是"用户当前没有正好开着这个聊天"）才会调用 `showNotification()`，避免在正在看的那个聊天里也弹一个多余的通知。`NotificationBanner`（`src/components/NotificationBanner.tsx`）挂载在 `App.tsx` 最外层（`.app-shell`内、`Routes`外面，所以不管在哪个页面都会显示），4秒自动消失，点击跳转到对应聊天。`previewForMessage()`（新提取到 `lib/messagePreview.ts`，`MessagesPage`的会话列表预览也改用这个共用函数了）负责把不同消息类型（text/sticker/link/commission/gift）变成一行预览文字。
...
分句发送+打字延迟：`typingDelayMs()` 按长度算延迟，`revealBubbles()`（现在住在 `lib/chatEngine.ts` 里，不在 `ChatPage` 组件里了，见上面"聊天引擎在后台运行"）用多个 `setTimeout` 依次落库。用户插话打断：模块级的streamId/定时器/AbortController，新消息一发就换新streamId、清旧定时器、abort旧请求——这个机制没变，只是不再挂在组件的ref上。
...
- `src/components/`：TopBar / BottomNav(5个tab) / SearchOverlay / MessageBubble(assistant消息头像上方显示名字，含委托卡片`CommissionCard`子组件+礼物卡片渲染) / NotificationBanner(全局通知横幅，挂在App.tsx最外层) / ActionSheet / Avatar(圆角矩形) / AvatarPicker / ImageCropper。
- `src/lib/`：**chatEngine.ts(核心！发消息+调API+揭示气泡的后台引擎，独立于ChatPage生命周期，见上面架构决策)** / deepseek.ts(含jsonMode开关+角色合并) / aiProtocol.ts(解析+非JSON兜底按行拆气泡+委托reward clamp+平衡括号JSON提取) / prompt.ts(三层提示词+人设生成) / memory.ts(记忆+关系增量，合并一次API调用) / relationship.ts(五维度/初始值/归纳标签) / shop.ts(独立商品生成prompt+解析) / wallet.ts(货币常量+格式化) / messagePreview.ts(消息类型→一行预览文字，会话列表和通知横幅共用) / randomTraits.ts(随机词条池) / contact.ts(displayName) / image.ts(图片压缩) / search.ts / time.ts(含当前时间描述+年龄计算) / colors.ts / avatarEmojis.ts。
- `src/store/`：useSettingsStore(persist，用户配置) / useChatEngineStore(不persist，每会话的aiTyping/error) / useChatUiStore(不persist，activeConversationId+通知横幅内容)。
